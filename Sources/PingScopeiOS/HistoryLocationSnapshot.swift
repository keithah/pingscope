...
public struct PingScopeIOSHistoryLocationStateMachine: Sendable {
    ...
    private func commands(
        ...
        if next.updatesActive != previous.updatesActive {
            commands.append(next.updatesActive ? startCommand(for: next) : .stopUpdatingLocation)
        } else if next.updatesActive, next.accuracy != previous.accuracy {
            commands.append(.stopUpdatingLocation)
            commands.append(startCommand(for: next))
        }
        return commands
    }

    private func startCommand(
        for policy: PingScopeIOSHistoryLocationPolicy
    ) -> PingScopeIOSHistoryLocationCommand {
        policy.accuracy == .keepAlive
