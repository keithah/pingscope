...
- [x] **MENU-01**: Menu bar shows color-coded status dot (green/yellow/red/gray)
- [x] **MENU-02**: Menu bar shows ping time in milliseconds below status dot
- [x] **MENU-03**: Left-click opens popover/window with full interface
- [x] **MENU-04**: Right-click (or Ctrl+click, Cmd+click) opens context menu
- [x] **MENU-05**: Context menu allows host switching
- [x] **MENU-06**: Context menu shows mode toggles (compact, stay-on-top)
- [x] **MENU-07**: Context menu includes Settings and Quit options

### Host Monitoring

- [x] **HOST-01**: Monitor multiple hosts simultaneously (Google DNS, Cloudflare, Gateway + custom)
- [x] **HOST-02**: Auto-detect default gateway via SystemConfiguration
- [x] **HOST-03**: Support ICMP ping method (simulated via TCP connections to ports 53, 80, 443, 22, 25)
- [x] **HOST-04**: Support UDP ping method (default port 53)
- [x] **HOST-05**: Support TCP ping method (default port 80)
- [x] **HOST-06**: Per-host configurable ping interval
- [x] **HOST-07**: Per-host configurable timeout
- [x] **HOST-08**: Per-host configurable thresholds (good/warning/error)
- [x] **HOST-09**: Host management (add, edit, delete custom hosts)
- [x] **HOST-10**: Default hosts cannot be deleted (Google, Cloudflare, Gateway)
- [x] **HOST-11**: True ICMP ping when running outside sandbox; ICMP option hidden when sandboxed

### Visualization

- [x] **VIS-01**: Real-time latency graph with line and gradient fill
- [x] **VIS-02**: Graph shows data points for individual pings
- [x] **VIS-03**: Graph has configurable time filter (1min, 5min, 10min, 1hour)
- [x] **VIS-04**: History table shows timestamp, host, ping time, status
- [x] **VIS-05**: History table scrollable with recent results first
- [x] **VIS-06**: Statistics display: transmitted, received, packet loss %
- [x] **VIS-07**: Statistics display: min/avg/max/stddev latency
...
- [x] **NOTF-01**: Request notification permission from user
- [x] **NOTF-02**: Alert when host transitions from good to no response
- [x] **NOTF-03**: Alert when ping exceeds configurable threshold
- [x] **NOTF-04**: Alert when host recovers from failure
- [x] **NOTF-05**: Alert on performance degradation (latency increases by X%)
- [x] **NOTF-06**: Alert on intermittent failures (N failures in M-ping window)
- [x] **NOTF-07**: Alert on network change (gateway IP changes)
- [x] **NOTF-08**: Alert when all hosts fail (internet loss)
- [x] **NOTF-09**: Per-host notification settings
- [x] **NOTF-10**: Global notification enable/disable
...
- [x] **TECH-01**: Use Swift Concurrency (async/await) throughout — no DispatchSemaphore
- [x] **TECH-02**: Proper NWConnection lifecycle management (no stale connections)
- [x] **TECH-03**: Accurate timeout handling (no false timeouts from race conditions)
- [x] **TECH-04**: Actor-isolated PingService for thread-safe concurrent pings
- [x] **TECH-05**: @MainActor ViewModels for UI thread safety
- [x] **TECH-06**: Support macOS 13.0+ (Ventura)
- [x] **TECH-07**: App Store sandbox compatible (no raw ICMP sockets)
