...
<task type="checkpoint:human-verify" gate="blocking">
  ...
  <files>N/A</files>
  <action>Export archived App Store build for manual testing of sandbox detection and TCP/UDP functionality</action>
  <verify>
Manual verification in running app:
1. ICMP method not visible in Settings → Hosts → Method dropdown
2. TCP ping to port 80 succeeds with latency updates
3. UDP ping to port 53 succeeds with latency updates
4. Console.app shows SandboxDetector logs indicating sandbox=true
5. codesign output confirms sandbox and network.client entitlements
  </verify>
  <done>
Exported App Store build runs in sandbox mode (ICMP hidden, TCP/UDP functional), SandboxDetector correctly detects sandboxed environment, and codesign verification confirms all required entitlements present.
  </done>
  <what-built>
