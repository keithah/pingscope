...
<task type="checkpoint:human-verify" gate="blocking">
  <what-built>
Screenshot capture automation script with interactive window selection and dimension validation.

User will run the script to capture five required screenshots:
1. Menu bar icon + full interface
2. Multi-host tabs + real-time graph
3. Settings panel
4. Ping history with statistics
5. Compact mode view

The script guides through each screenshot, validates dimensions (2880x1800), and opens each in Preview for immediate review.
  </what-built>
  <how-to-verify>
**Setup:**
1. Ensure display is set to native resolution (System Settings > Displays > Use As: Default for display)
2. Launch PingScope app (use App Store archive from Phase 14-03 or build PingScope-AppStore scheme)
3. Prepare app states for each screenshot:
...
For each screenshot prompt:

**Screenshot 1: Menu bar + full interface**
- Open full mode interface (click menu bar icon)
- Position window center-screen
- Show menu bar at top with PingScope icon visible
- Ensure status shows active monitoring (green/yellow indicator)
- Click window when prompted

**Screenshot 2: Multi-host tabs + graph**
- Switch to a host with populated graph data
- Ensure graph shows visible latency curve (not empty)
- Show multiple host tabs at top
- Time range selector visible
- Click window when prompted

**Screenshot 3: Settings panel**
- Open Settings (Cmd+, or menu)
- Show Hosts tab OR Notifications tab (choose most visually clear)
- Ensure settings content is readable
- Click window when prompted

**Screenshot 4: Ping history with statistics**
- Switch to full mode
- Scroll history table to show multiple entries
- Ensure status column shows mix of Success/Failed states
- Ensure timestamp, latency, and status columns visible
- Click window when prompted

**Screenshot 5: Compact mode**
- Toggle to compact mode (Display > Compact Mode in menu or settings)
- Position compact window center-screen
- Ensure latency value and status indicator visible
- Show compact layout clearly
- Click window when prompted

**Verify captured screenshots:**
- Preview opens automatically for each screenshot
- Check clarity, composition, no clipping
- Verify all UI elements are readable
- Re-run script if any screenshot needs adjustment
...
All screenshots should show: pixelWidth: 2880, pixelHeight: 1800

**Expected outcome:**
- Five screenshots exist in AppStoreAssets/Screenshots/
- All dimensions are 2880x1800
...
If issues, describe which screenshots need adjustment (e.g., "Screenshot 2 graph is empty, need more data" or "Screenshot 3 settings window clipped").
  </resume-signal>
</task>
