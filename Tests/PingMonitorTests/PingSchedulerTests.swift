...
final class PingSchedulerTests: XCTestCase {
    func testShorterIntervalHostProducesMoreResultsOverSameWindow() async {
        let fastHost = PingMonitor.Host(name: "Fast", address: "fast.local", intervalOverride: .milliseconds(120))
        let slowHost = PingMonitor.Host(name: "Slow", address: "slow.local", intervalOverride: .milliseconds(320))
    ...
    func testNilIntervalOverrideUsesProvidedGlobalFallback() async {
        let host = PingMonitor.Host(name: "Fallback", address: "fallback.local", intervalOverride: nil)
