...
`DefaultGatewayEndpointResolver` still races its candidates and returns the first successful candidate. Its result carries an existing host configuration whose method and port identify the actual successful probe. Callers pass that resolved `HostConfig` through unchanged, and a focused test forces a lower-priority candidate to answer first. No preferred-order waiting or protocol reorder is introduced.

## Incident digest indexing

The onset-diagnosis initializer builds one chronologically sorted sequence per host and maintains per-host cursors as focused-host samples advance. At each failure onset, each cursor advances only forward until its timestamp is at or before the onset. The latest known sample is then used for the existing `NetworkPerspectiveDiagnoser` call. This replaces repeated binary searches with a linear pass over each bounded host sequence while preserving the same latest-sample selection rule.
...
- History: existing incident cases remain unchanged, and an interleaved multi-host onset fixture demonstrates cursor-based selection matches the legacy latest-sample semantics.
